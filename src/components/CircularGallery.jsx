...
import styles from '@/styles/CircularGallery.module.scss'
...
class Media {
  ...
  onResize({ screen, viewport } = {}) {
    ...
    // Make cards square (1:1 ratio) for NFT format
    ...
    this.padding = 1.5; // Reduced padding for square NFT cards
...
class App {
  ...
  onCheck() {
    ...
    // Calculate current group (every 3 items)
    const originalLength = this.mediasImages.length / 2; // Remove duplication
    const normalizedIndex = itemIndex % originalLength;
    const newGroup = Math.floor(normalizedIndex / 3);
    
    if (newGroup !== this.currentGroup && this.onGroupChange) {
...
// NFT Images organized in groups of 3 - agora buscando em /img/
const nftGroups = [
...
// Background themes for each group (agora apenas 3)
const backgroundThemes = [
  'afrika', 'asian', 'american'
];
